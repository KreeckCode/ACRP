...
class Event(models.Model):
    ...
    class Meta:
        ordering = ["-start_time"]
        permissions = [("manage_events", "Can create, update, and delete events")]
...
class Announcement(models.Model):
    ...
    posted_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="posted_announcements"
    )
...
class Projects(models.Model):
    ...
    attachment = models.FileField(
        upload_to="project_attachments/", blank=True, null=True
    )
    ...
    def is_active(self):
        ...
        return self.status in ["IN_PROGRESS", "ON_HOLD"] and (
            self.start_date
            <= timezone.now().date()
            <= (self.end_date or timezone.now().date())
        )

    class Meta:
        ordering = ["-start_date"]
        permissions = [("manage_projects", "Can create, update, and delete projects")]
...
class Quiz(models.Model):
    ...
    resource = models.OneToOneField(
        ...
        related_name="quiz",
        limit_choices_to={"resource_type": "QUIZ"},
    )
