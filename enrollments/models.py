...
# — DESIGNATED AFFILIATION —
class DesignatedAffiliation(models.Model):
    approved       = models.BooleanField(default=False, editable=False)
    approved_at    = models.DateTimeField(null=True, blank=True, editable=False)
    approved_by    = models.ForeignKey(
        User, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='approved_designated_affiliations',
        editable=False
    )
    created_user   = models.ForeignKey(
        User, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='designated_enrollments',
        editable=False
    )

    title           = models.CharField(max_length=20)
    gender          = models.CharField(max_length=10, choices=AssociatedAffiliation.GENDER_CHOICES)
    surname         = models.CharField(max_length=50)
    initials        = models.CharField(max_length=10)
    ...
    occupation       = models.CharField(max_length=100, blank=True)
    date_commenced   = models.DateField(blank=True)
    institution      = models.CharField(max_length=200,blank=True)
    accountable_to   = models.CharField(max_length=100,blank=True)
    years_ministry   = models.PositiveIntegerField(default=0)
    months_ministry  = models.PositiveIntegerField(default=0)
    work_description = models.TextField(blank=True)
    ...
    boards_organisations     = models.TextField(blank=True)

    documents = GenericRelation(Document)

    def __str__(self):
        return f"DesignatedAffiliation({self.first_name} {self.last_name})"
...
class StudentAffiliation(models.Model):
    ...
    approved_by    = models.ForeignKey(
        User, null=True, blank=True,
        on_delete=models.SET_NULL, related_name='approved_student_affiliations',
        editable=False
    )
    created_user   = models.ForeignKey(
        User, null=True, blank=True,
    ...
    def __str__(self):
        return f"StudentAffiliation({self.first_name} {self.last_name})"
